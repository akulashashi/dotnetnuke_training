...
namespace bhattji.Modules.XYZ70s
{
    ...
    public class XYZ70Info
    {
        ...
        [Ignore]
        public string PublishDateString { get { return PublishDate.HasValue ? Convert.ToDateTime(PublishDate).ToShortDateString() : string.Empty; } set { try { if (!string.IsNullOrEmpty(value) && (Convert.ToDateTime(value).Year > 1752)) PublishDate = Convert.ToDateTime(value); } catch { } } }
        public DateTime? ExpiryDate { get; set; }
        [Ignore]
        public string ExpiryDateString { get { return ExpiryDate.HasValue ? Convert.ToDateTime(ExpiryDate).ToShortDateString() : string.Empty; } set { try { if (!string.IsNullOrEmpty(value) && (Convert.ToDateTime(value).Year > 1752)) ExpiryDate = Convert.ToDateTime(value); } catch { } } }
        [ResultColumn]
        public DateTime DisplayDate { get; set; }
        public int? ViewOrder { get; set; }
        [Ignore]
        public string ViewOrderString { get { return ViewOrder.HasValue ? ViewOrder.ToString() : string.Empty; } set { try { if (!string.IsNullOrEmpty((value))) ViewOrder = Convert.ToInt32(value); } catch { } } }
    ...
    public class XYZ70sController : CommonControllerMethods, ISearchable, IPortable, IUpgradeable
    {
        ...
        {
            string strSql = "SELECT TOP 1 ItemId FROM " + GetDboName("XYZ70s") + " WHERE Title = " + Title;
            return db.ExecuteScalar<int>(strSql);
        }
        ...
        public bool UpdateXYZ70s(string XlsCsvTxtFile)
        {
            ...
            try
            {
                ...
                List<XYZ70Info> lot = ImportFromFile(XlsCsvTxtFile).ToLOT();
        ...
        public string ExportModule(int ModuleID)
        {
            ...
            sbXML.Append("<XYZ70s>");
            IEnumerable<XYZ70Info> lotXYZ70s = GetModuleXYZ70s(ModuleID);
        ...
        public void ImportModule(int ModuleID, string Content, string Version, int UserId)
        {
            ...
            foreach (XmlNode xmlXYZ70 in xmlXYZ70s.SelectNodes("XYZ70"))
            {
                ...
                //try { objXYZ70.Category = xmlXYZ70["Category"].InnerText; } catch { }
            ...
            try { objOI.OnlyEmbedTag = bool.Parse(xmlXYZ70s.SelectSingleNode("OnlyEmbedTag").InnerText); }
            catch { }
            try { objOI.ShowControls = bool.Parse(xmlXYZ70s.SelectSingleNode("ShowControls").InnerText); }
            catch { }
            try { objOI.SSTitle = xmlXYZ70s.SelectSingleNode("SSTitle").InnerText; }
            catch { }
            try { objOI.CaptionFont = xmlXYZ70s.SelectSingleNode("CaptionFont").InnerText; }
            catch { }
            try { objOI.CaptionSize = int.Parse(xmlXYZ70s.SelectSingleNode("CaptionSize").InnerText); }
            catch { }
            try
            {
